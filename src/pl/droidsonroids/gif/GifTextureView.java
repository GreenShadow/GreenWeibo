...
public class GifTextureView extends TextureView {
	...
	private void init(AttributeSet attrs, int defStyleAttr, int defStyleRes) {
		if (attrs != null) {
			...
			final TypedArray textureViewAttributes = getContext().obtainStyledAttributes(attrs,
					R.styleable.GifTextureView, defStyleAttr, defStyleRes);
		...
		if (value.resourceId != 0) {
			...
			if ("drawable".equals(type) || "raw".equals(type)) {
				...
				throw new IllegalArgumentException(
						"Expected string, drawable or raw resource, type " + type + " " + "cannot be converted to GIF");
			}
		}
	...
	private static class RenderThread extends Thread implements SurfaceTextureListener {
		...
		RenderThread(final GifTextureView gifTextureView) {
			...
			mGifTextureViewReference = new WeakReference<GifTextureView>(gifTextureView);
		}
