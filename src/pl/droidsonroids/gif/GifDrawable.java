 ...
 * animations. Basic GIF metadata can also be examined.
 *
 * @author koral--
 */
public class GifDrawable extends Drawable implements Animatable, MediaPlayerControl {
	 ...
	 * @throws IOException
	 *             if buffer does not contain valid GIF data or is indirect
	 * @throws NullPointerException
	 *             if buffer is null
	 */
	...
	/**
	 * Creates drawable from {@link android.net.Uri} which is resolved using
	 * {@code resolver}.
	 * {@link android.content.ContentResolver#openAssetFileDescriptor(android.net.Uri, String)}
	 * is used to open an Uri.
	 *
	 * @param uri
	 *            GIF Uri, cannot be null.
	 * @param resolver
	 *            resolver used to query {@code uri}, can be null for file://
	 *            scheme Uris
	 * @throws IOException
	 *             if resolution fails or destination is not a GIF.
	 */
	...
	/**
	 * Causes the animation to start over. If rewinding input source fails then
	 * state is not affected. This method is thread-safe.
	 */
	public void reset() {
	 ...
	 * block. Defaults to 1 if there is no such extension.
	 *
	...
	/**
	 * Sets loop count of the animation. Loop count must be in range &lt;0
	 * ,65535&gt;
	 *
	 * @param loopCount
	 *            loop count, 0 means infinity
	 */
	 ...
	 * If position is greater than duration of the loop of animation (or whole
	 * animation if there is no loop) then animation will be sought to the end.
	 * <br>
	 * NOTE: all frames from current (or first one if seeking backward) to
	 * desired one must be rendered sequentially to perform seeking. It may take
	 * a lot of time if number of such frames is large. Method is thread-safe.
	 * Decoding is performed in background thread and drawable is invalidated
	 * automatically afterwards. If position exceeds animation duration, seek
	 * stops at the end, no exception is thrown.
	 *
	 * @param position
	...
	/**
	 * Like {@link #seekTo(int)} but uses index of the frame instead of time. If
	 * frameIndex exceeds number of frames, seek stops at the end, no exception
	 * is thrown.
	 *
	 * @param frameIndex
	...
	/**
	 * Like {@link #seekToFrame(int)} but performs operation synchronously and
	 * returns that frame.
	 *
	 * @param frameIndex
	 *            index of the frame to seek to (zero based)
	...
	/**
	 * Like {@link #seekTo(int)} but performs operation synchronously and
	 * returns that frame.
	 *
	 * @param position
	 *            position to seek to in milliseconds
	 ...
	 * <p>
	 * This method should not be used to calculate the memory usage of the
	 * bitmap. Instead see {@link #getAllocationByteCount()}.
	 *
	 * @return the minimum number of bytes that can be used to store pixels of
	 *         the single frame
	 */
	 ...
	 * length cannot be determined. Returned value does not change during
	 * runtime. If GifDrawable is constructed from {@link InputStream} -1 is
	 * always returned. In case of byte array and {@link ByteBuffer} length is
	 * always known. In other cases length -1 can be returned if length cannot
	 * be determined.
	 *
	 ...
	 * @throws IllegalArgumentException
	 *             if x, y exceed the drawable's bounds
	 * @throws IllegalStateException
	 *             if drawable is recycled
	 */
	public int getPixel(int x, int y) {
		if (x >= mNativeInfoHandle.width) { // need to check explicitly because
											// reused bitmap may be larger
	...
	protected void onBoundsChange(Rect bounds) {
		...
		if (shader != null) {
			...
			shaderMatrix.preScale(mDstRectF.width() / mBuffer.getWidth(), mDstRectF.height() / mBuffer.getHeight());
	 ...
	 *
	 * @param listener
	 *            animation listener to be added, not null
	 ...
	 *
	 * @param listener
	 *            animation listener to be removed
	...
	/**
	 * Sets whether this drawable is visible. If rendering of next frame is
	 * scheduled on draw current one (the default) then this method only calls
	 * through to the super class's implementation.<br>
	 * Otherwise (if
	 * {@link GifDrawableBuilder#setRenderingTriggeredOnDraw(boolean)} was used
	 * with <code>true</code>) when the drawable becomes invisible, it will
	 * pause its animation. A subsequent change to visible with
	 * <code>restart</code> set to true will restart the animation from the
	 * first frame. If <code>restart</code> is false, the animation will resume
	 * from the most recent frame.
	 *
	 * @param visible
	 *            true if visible, false otherwise
	 * @param restart
	 *            when visible and rendering is triggered on draw, true to force
	 *            the animation to restart from the first frame
	...
	/**
	 * Returns zero-based index of recently rendered frame in given loop or -1
	 * when drawable is recycled.
	 *
	...
	/**
	 * Returns zero-based index of currently played animation loop. If animation
	 * is infinite or drawable is recycled 0 is returned.
	 *
	...
	/**
	 * Returns whether all animation loops has ended. If drawable is recycled
	 * false is returned.
	 *
	...
	/**
	 * Returns duration of the given frame (in milliseconds). If there is no
	 * data (no Graphics Control Extension blocks or drawable is recycled) 0 is
	 * returned.
	 *
	 * @param index
	 *            index of the frame
	 ...
	 * @throws IndexOutOfBoundsException
	 *             if index &lt; 0 or index &gt;= number of frames
	 */
	...
	/**
	 * Sets the corner radius to be applied when drawing the bitmap. Note that
	 * changing corner radius will cause replacing current {@link Paint} shader
	 * by {@link BitmapShader}.
	 * 
	 * @param cornerRadius
	 *            corner radius or 0 to remove rounding
	 */
	...
	/**
	 * @return The corner radius applied when drawing this drawable. 0 when
	 *         drawable is not rounded.
	 */
