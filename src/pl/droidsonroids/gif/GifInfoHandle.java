...
final class GifInfoHandle {
	...
	/**
	 * Pointer to native structure. Access must be synchronized, heap corruption
	 * may occur otherwise when {@link #recycle()} is called during another
	 * operation.
	 */
	...
	static native GifInfoHandle openFd(FileDescriptor fd, long offset, boolean justDecodeMetaData)
	...
	static native GifInfoHandle openDirectByteBuffer(ByteBuffer buffer, boolean justDecodeMetaData)
			throws GifIOException;
	...
	static GifInfoHandle openAssetFileDescriptor(AssetFileDescriptor afd, boolean justDecodeMetaData)
			throws IOException {
		try {
	...
	static GifInfoHandle openUri(ContentResolver resolver, Uri uri, boolean justDecodeMetaData) throws IOException {
		if (ContentResolver.SCHEME_FILE.equals(uri.getScheme())) // workaround
																	// for #128
		{
